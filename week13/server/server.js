...
const isLoggedIn = (req, res, next) => { //da mettere in tutte le API
...
//lo scrivo per la get ma vale per tutte le api...
//...
// GET /api/courses
...
//lo scrivo per la get ma vale per tutte in generale (vale tutte le volte che ho a che fare con le operazioni relative ad un utente autenticato (getu, put, post, delete))...
//...non possiamo mettere i dati dell'user nell'url (OCCHIO, PUNTI ALLESAME)...
//...non può essere il client a decidere su quale utente recuperare i dati...
//...ma lo decide il server in base della sessione 
// GET /api/exams
app.get('/api/exams', isLoggedIn, async (req, res) => {
//app.get('/api/exams/:userId', isLoggedIn, async (req, res) => { NOOOOOOOOOOOOOOOOO
  try {
    //const exams = await dao.listExamsWithName(req.params.userId); NOOOOOOOOOOO
